...
export default function VendorAnalyticsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [
        ...
        db.from('vendor_products').select('id, name, price_per_day, currency').eq('vendor_id', vp.id).eq('is_active', true),
      ...
      setData({ allOrders, revenue, pending, confirmed, topProducts, monthlyMap, totalProducts: totalProducts || 0 })
      setLoading(false)
    }
    load()
  }, [db])
  ...
  const maxRevenue = Math.max(...Object.values(data.monthlyMap as Record<string, number>), 1)

  return (
    <div className="p-8 text-white">
      <div className="mb-8">
        <h1 className="font-display font-extrabold text-2xl text-white">Analytics</h1>
        <p className="text-white/40 text-sm mt-1">Performance overview of your rental business</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Revenue',    value: `₹${data.revenue.toLocaleString()}`, icon: Wallet,     color: 'text-green-400' },
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 border border-white/8 rounded-xl p-6">
          <h2 className="font-display font-bold text-white text-sm mb-5">Monthly Revenue</h2>
              ...
              {Object.entries(data.monthlyMap as Record<string, number>).slice(-6).map(([month, rev]) => (
                <div key={month} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-white/5 rounded-full h-2">
                    <div className="bg-gold h-2 rounded-full transition-all"
                      style={{ width: `${(rev / maxRevenue) * 100}%` }} />
                  </div>
                  <span className="text-[12px] font-medium text-white w-20 text-right">₹{rev.toLocaleString()}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="space-y-3">
              {data.topProducts.map((p: any, i: number) => (
                <div key={p.name} className="flex items-center gap-3">
                  ...
                  <span className="text-[12.5px] font-semibold text-green-400">₹{p.revenue.toLocaleString()}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white/5 border border-white/8 rounded-xl p-6">
          ...
          <div className="space-y-4">
            {[
              ...
              { label: 'Pending Value',     value: `₹${data.allOrders.filter((o: any) => o.status === 'pending').reduce((s: number, o: any) => s + o.total, 0).toLocaleString()}`, color: 'text-yellow-400' },
              ...
              { label: 'Avg Order Value',   value: data.allOrders.length ? `₹${Math.round(data.allOrders.reduce((s: number, o: any) => s + o.total, 0) / data.allOrders.length).toLocaleString()}` : '—', color: 'text-blue-400' },
            ].map(s => (
