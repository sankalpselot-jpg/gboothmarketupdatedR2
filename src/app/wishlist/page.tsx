...
export default function WishlistPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('is_active', true)
      .then(({ data }) => {
        setProducts((data as (Product & { categories: Category })[]) || [])
        setLoading(false)
      })
  }, [ids])
        ...
        <div className="max-w-[1100px] mx-auto">
          ...
          {!loading && ids.length === 0 && (
            <div className="card p-16 text-center">
              ...
              <p className="text-[#6B6B6B] mb-6">Save items you're interested in — click the ♡ on any product.</p>
          ...
          {!loading && products.length > 0 && (
            <>
              <p className="text-[13px] text-[#6B6B6B] mb-6">{products.length} saved item{products.length !== 1 ? 's' : ''}</p>
                  ...
                  return (
                    <div key={p.id} className="card overflow-hidden">
                      ...
                      <div className="p-4">
                        ...
                        <Link href={`/products/${p.slug}`}>
                          <h3 className="font-display font-semibold text-navy text-[15px] mb-3 leading-snug hover:text-gold transition-colors">{p.name}</h3>
                        </Link>
